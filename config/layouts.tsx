...
const articles: IArticle[] = articleServices();
...
// Corrected dynamic article grid items
articles.forEach((article, index) => {
  gridItems.push({
    i: `article-${index}`, // Using template literal with backticks
    component: () => <ArticleCard {...article} /> // JSX now properly parsed in .tsx file
});
  ...
  mdLayout.push({ i: `article-${index}`, x, y, w: 2, h: 2 });
  smLayout.push({ i: `article-${index}`, x: 0, y: smBaseY + index, w: 2, h: 2 });
});
